...
const BUSINESS_EMAIL = 'hello.lmtechnology@gmail.com';
...
const EJS_NOTIFY_LEAD_TEMPLATE = import.meta.env.VITE_EMAILJS_NOTIFY_LEAD_TEMPLATE_ID || EJS_CONTACT_TEMPLATE;
...
export const sendContactNotification = (form, lang) => {
  ...
  const leadParams = {
    ...
    subject: copy.contactLeadSubject,
    form_type: 'contact',
  };
  ...
  return Promise.all([
    sendEmail(EJS_CONTACT_TEMPLATE, leadParams),
    sendEmail(EJS_CONFIRMATION_TEMPLATE, confirmationParams),
  ]);
};
...
export const sendAvailabilityNotification = ({ email, lang, source }) => {
  ...
  const leadParams = {
    ...
    email,
    name: email,
    subject: copy.notifyLeadSubject,
    message: `${email} quiere recibir aviso de ${source}.`,
    source,
  ...
  return Promise.all([
    sendEmail(EJS_NOTIFY_LEAD_TEMPLATE, leadParams),
    sendEmail(EJS_NOTIFY_TEMPLATE, confirmationParams),
  ]);
};
